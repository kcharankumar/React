 ...
 class Tutorials
{
   ...
   constructor (id, title, description, published, submitted)
   {
       this.id = id;
       this.title = title;
...
export default class AddTutorial extends Component
{
    ...
    saveTutorial()
    {
         var data = { title: this.state.title, description:this.state.description};
              ...
              var data1 = new Tutorials(1, data.title, data.description, false, true);
        //  const data1 = new Tutorials(1, "Introduction to Java", "This gives information about java..", false, true);
     
        TutorialDataService.create (data1).then (response => 
            {
                this.setState({
              id:response.data.id,
